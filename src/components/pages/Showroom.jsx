...
import { vehicles } from '../../data/vehiclesData'; // Your vehicle data
...
export const auctionVehicleData = {
  ...
  model: "Senna",
  year: 2021,
  imageUrl: McLarenSennaImage, // Now using the imported local image
  openingBid: 850000,
...
const Showroom = () => {
  ...
  const featuredAuctionRef = useRef(null);
  ...
  const uniqueBodyTypes = [...new Set(vehicles.map(v => v.bodyType || 'Other'))];
  const uniqueTransmissions = [...new Set(vehicles.map(v => v.transmission))];
  const uniqueFuelTypes = [...new Set(vehicles.map(v => v.fuelType))];

  // --- FIX: Separate useEffect for initial URL param setup ---
  // This effect will run once on mount or when searchParams/uniqueMakes change,
  // setting the initial filter state without causing an infinite loop.
  useEffect(() => {
    const urlMake = searchParams.get('filter');
    if (urlMake && uniqueMakes.includes(urlMake) && urlMake !== makeFilter) {
      setMakeFilter(urlMake);
    }
    // You can add more logic here for other initial URL filters if needed
  }, [searchParams, uniqueMakes, makeFilter]); // makeFilter included to prevent re-setting if already matched

  // --- Main Filtering Logic and Animation useEffect ---
  ...
  useEffect(() => {
    ...
    currentVehicles = currentVehicles.filter(vehicle => {
      ...
      if (mileageFilter) {
        const vehicleMileageNum = parseInt(vehicle.mileage.replace(/,/g, ''), 10);
    ...
    // Animate vehicle cards on filter change
    if (vehicleGridRef.current) { // Added null check for ref
      gsap.fromTo(vehicleGridRef.current.children,
        { autoAlpha: 0, y: 50 },
        { autoAlpha: 1, y: 0, stagger: 0.05, duration: 0.6, ease: 'power3.out' }
      );
    }
  }, [makeFilter, priceRangeFilter, yearFilter, mileageFilter, bodyTypeFilter, transmissionFilter, fuelTypeFilter, vehicles]); // Dependencies are filter states + base vehicles data


  // Countdown timer effect (no changes needed)
  useEffect(() => {
  ...
  // GSAP animations for initial load (UPDATED from gsap.from to gsap.fromTo)
  useEffect(() => {
    console.log('Animating filter bar & featured auction…'); // just to confirm this runs

    if (filterBarRef.current) {
      gsap.fromTo(
        filterBarRef.current,
        { autoAlpha: 0, y: -50 }, // FROM this state
        { autoAlpha: 1, y: 0, duration: 0.8, ease: 'power3.out', delay: 0.2 } // TO this state
      );
    }
    ...
    if (featuredAuctionRef.current) {
      gsap.fromTo(
        ...
        { autoAlpha: 0, y: 50 }, // FROM this state
        { autoAlpha: 1, y: 0, duration: 0.8, ease: 'power3.out', delay: 0.4 } // TO this state
      );
    }
  }, []);
  ...
  const getSpecialFeatureSVG = (vehicle) => {
    if (vehicle.make === 'Mercedes-Benz' && vehicle.model.includes('AMG')) return <SVGGearbox />;
  ...
  return (
    <div className="showroom-page">
      ...
      {/* Mobile Filter Button (moved outside and styled to show only on mobile) */}
      ...
      {/* Filter Bar - Desktop/Tablet (REMOVED only-on-desktop CLASS) */}
        ...
        {filteredVehicles.length > 0 ? (
          filteredVehicles.map(vehicle => (
            <VehicleCard
              ...
              vehicle={{
                ...vehicle,
                // Add a dummy bodyType if it doesn't exist for filtering purposes
                bodyType: vehicle.bodyType || (vehicle.make === 'Rolls-Royce' ? 'SUV' : (vehicle.make === 'Mercedes-Benz' && vehicle.model.includes('G 63') ? 'SUV' : (['911', '488 GTB', 'Huracán EVO', 'R8'].some(m => vehicle.model.includes(m)) ? 'Coupe' : 'Sedan')))
              }}
